...
export default function AdminDashboard(){
  const [getAllNews,setGetAllNews] = React.useState({
    newsCount:null
  })
  const [getAllCat,setGetAllCat] = React.useState(
    {
      categoryCount:null
    }
  )
  ...
  const getAllCount =()=>{
    ...
    .then((res)=>{
      // const Cnews = 
       ...
       // const Ccategory= 
       setGetAllNews({newsCount:newsCount})
       const categoryCount = res.data.getCategoryCount;
        setGetAllCat({categoryCount:categoryCount})
        
  
      // const Cuser =
       const userCount =  res.data.getUserCount;
       setGetAllUser({userCount:userCount})
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 p-4 gap-4">
          <div className="bg-blue-500 dark:bg-gray-800 shadow-lg rounded-md flex items-center justify-between p-3 border-b-4 border-blue-600 dark:border-gray-600 text-white font-medium group">
            ...
            <div className="text-right">
              <p className="text-2xl">{getAllUser.userCount}</p>
              <p>Users</p>
            </div>
          </div>
          <div className="bg-blue-500 dark:bg-gray-800 shadow-lg rounded-md flex items-center justify-between p-3 border-b-4 border-blue-600 dark:border-gray-600 text-white font-medium group">
            ...
            <div className="text-right">
              <p className="text-2xl">{getAllCat.categoryCount}</p>
              <p>Categoires</p>
            </div>
          </div>
          <div className="bg-blue-500 dark:bg-gray-800 shadow-lg rounded-md flex items-center justify-between p-3 border-b-4 border-blue-600 dark:border-gray-600 text-white font-medium group">
            ...
            <div className="text-right">
              <p className="text-2xl">{getAllNews.newsCount}</p>
              <p>News</p>
            </div>
          </div>
